  ...
  class << self
    ...
    def draw
      ...
      Window.draw_font(x, 0, "MENU", @@font_big)
            .draw_font((@@win_w - @@font_jp.get_width("設定")) / 2, 220, "設定", @@font_jp)
            .draw_font(@@win_w * 0.2, @@button_y, "AUTO_ATTACK", @@font)
            .draw_font(@@button_on_x,  @@button_y, "ON", @@font, color: $p_set.auto_attack ? C_WHITE : [100, 255, 255, 255])
            .draw_font(@@button_off_x, @@button_y, "OFF", @@font, color: !$p_set.auto_attack ? C_WHITE : [100, 255, 255, 255])
            .draw_font((@@win_w - @@font_jp.get_width("操作方法")) / 2, 480, "操作方法", @@font_jp)
