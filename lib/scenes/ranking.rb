class Ranking < Scene
  def initialize(user_name, score)
    ...
    @@user_db.each_with_index do |item, i|
      ...
      name_img =  Image.new(w[0], @@font_big.size   * 0.8).draw_font(10, -30, "#{name}", @@font_big)
      score_img = Image.new(w[1], @@font_nomal.size * 0.8).draw_font(10, -20, "#{score}", @@font_nomal)
      time_img =  Image.new(w[2], @@font_mini.size       ).draw_font(10, -10, "#{time}", @@font_mini, [240, 240, 240])
